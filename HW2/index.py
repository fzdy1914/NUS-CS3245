...
def build_index(in_dir, out_dict, out_postings):
    ...
    # create skip pointer indices for any possible length of the whole set of docs
    # length is the number of docs
    def create_skip_pointers(length):
        skip_pointers_list = list()  # store skip pointers indices

        for i in range(0, length+1):
            if i <= 1:
                skip_pointers_list.append(list())
            else:
                num_pointers = int(math.sqrt(i))
                space = int((i - 1) / num_pointers)
                skip_pointers_list.append(list(range(0, i, space)))

        return skip_pointers_list
    ...
    #  initially we only have to load the first item in each block to memory
    ...
    while pq.qsize() > 0:
        ...
        # when the term has been merged from all blocks
        if term != current_term:
            # save posting for the term to disk
    ...
    create_skip_pointers(len(final_doc_id_set))
    ...
    pickle.dump(create_skip_pointers(len(final_doc_id_set)), out_dictionary_file)
